...
import { BatchTableConfig } from "@/config/tables/batch-table";

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Manage Batches', href: '/batches' },
];
...
interface Batch {
    id: number;
    batch_name: string;
    curriculum: string; // display curriculum name
    start_date: string; 
...
export default function Index({ batches, filters, totalCount, filteredCount }: IndexProps) {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        router.get(route('batches.index'), queryString, {
            preserveState: true,
            preserveScroll: true,
        });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                ...
                <div className="mb-4 flex w-full items-center justify-between gap-4">
                    ...
                    <div className="ml-auto">
                        ...
                        >
                            <CirclePlusIcon className="me-2" /> Add Batch
                        </Link>
                    </div>
                </div>
                ...
                <CustomTable
                    ...
                    actions={BatchTableConfig.actions}
                    data={batches.data}
                    from={batches.from - 1}
                    isModal={false}
                    onDelete={handleDelete}
                    onView={(row) => router.get(route('batches.show', row.id))}
                    onEdit={(row) => router.get(route('batches.edit', row.id))}
                />
